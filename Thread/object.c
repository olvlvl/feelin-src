...
GOFROMIEL @ YOMGUI: Est-ce que je peux  virer  tout  ça  maintenant  que  je
n'utilise plus de registres pour sauter dans le vrai code du thread ?? Parce
que j'aimerai beaucoup faire ça ;-)
                    
                    #ifdef __MORPHOS__

                    REG_A0 = (uint32) (Obj);
                    REG_A1 = (uint32) (userdata);
                    REG_A2 = (uint32) (&LOD->pub);
                    (*MyEmulHandle->EmulCallDirect68k)(entry);
                    death_msg = (FThreadMsg *) REG_D0;

                    #else

                    death_msg = entry(Obj, userdata, &LOD->pub);
...
F_METHOD(FObject, Thread_New)
{
    ...
    uint32 entry = 0, stacksize = 8192, pri = 0;
    STRPTR name = NULL;
    APTR data = NULL;
    ...
    while (IFEELIN F_DynamicNTI(&Tags, &item, Class) != NULL)
    {
        switch (item.ti_Tag)
        {
            case FV_ATTRIBUTE_ENTRY: entry = item.ti_Data; break;
            ...
            case FV_ATTRIBUTE_USERDATA: data = (APTR) item.ti_Data; break;
        }
    }
    ...
    if (entry)
    {
        ...
        if (LOD->pub.Process)
        {
            ...
            (
                ...
                FV_Thread_Init, F_MSG_THREAD_INIT(Class, Obj, entry, data)
            );
