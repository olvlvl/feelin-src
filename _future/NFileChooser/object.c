...
void filechooser_select_disabled(FClass *Class,FObject Obj,struct FeelinBase *FeelinBase)
{
    ...
    {
        if (!F_Get(LOD->icons,FA_Widget_Disabled))
        {
            F_Do(Obj,FM_MultiSet,FA_Widget_Disabled,TRUE,LOD->icons,LOD->volumes,LOD->parent,LOD->match,NULL);
        }
    }
    else
    {   
        if (F_Get(LOD->icons,FA_Widget_Disabled))
        {
            F_Do(Obj,FM_MultiSet,FA_Widget_Disabled,FALSE,LOD->icons,LOD->volumes,LOD->parent,LOD->match,NULL);
        }
    }
}
//+
...
F_METHOD(FObject,FileChooser_New)
{
    ...
    if (F_SuperDo(Class,Obj,Method,
        ...
        Child, ListviewObject, FA_ChainToCycle,FALSE, FA_Widget_Weight,25, /*FA_Area_MinHeight,100,*/
            "FA_Listview_List", LOD->list_volumes = ListObject, FA_Widget_SetMin,FV_Widget_SetWidth, FA_Widget_SetMax,FV_Widget_SetWidth,
...
//        Child, BalanceObject, End,
        ...
        Child, VGroup,
            Child, HGroup,
                Child, ImageObject, "FA_Image_Spec",DEF_FILECHOOSER_IMAGE_HOME, FA_Widget_SetMax,FV_Widget_SetBoth, End,
                Child, ImageObject, "FA_Image_Spec",DEF_FILECHOOSER_IMAGE_BACK, FA_Widget_SetMax,FV_Widget_SetBoth, End,
                Child, ImageObject, "FA_Image_Spec",DEF_FILECHOOSER_IMAGE_FORWARD, FA_Widget_SetMax,FV_Widget_SetBoth, End,
                Child, AreaObject, FA_MaxHeight,2, End,
            End,
            
            Child, ListviewObject, FA_ChainToCycle,FALSE, FA_Area_MinHeight,100,
                "FA_Listview_List", LOD->list_files = ListObject, End,
            End,
/*
            Child, HGroup, FA_Widget_SetMax,FV_Widget_SetHeight,
                Child, LOD->file = StringObject, "AdvanceOnCR",FALSE, FA_Widget_Weight,80, End,
                Child, LOD->balance = BalanceObject, End,
                Child, LOD->pattern = StringObject, "AdvanceOnCR",FALSE, FA_Widget_Weight,20, End,
                Child, LOD->match = F_MakeObj(FV_MakeObj_Button,"?", FA_ChainToCycle,FALSE, FA_Widget_Mode,FV_Widget_Mode_Toggle, FA_Widget_SetMax,FV_Widget_SetBoth, TAG_DONE),
                Child, LOD->icons = F_MakeObj(FV_MakeObj_Button,"i", FA_ChainToCycle,FALSE, FA_Widget_Mode,FV_Widget_Mode_Toggle, FA_Widget_SetMax,FV_Widget_SetBoth, TAG_DONE),
            End,
*/
        End,
...
/*
    ...
    if (F_SuperDo(Class,Obj,Method,
        ...
        Child, ListviewObject, FA_ChainToCycle,FALSE, FA_Area_MinHeight,100, "FA_Listview_List",
            LOD->list = DOSListObject, End,
        End,
        
        Child, HGroup,
            Child, LOD->volumes = F_MakeObj(FV_MakeObj_Button,":",FA_ChainToCycle,FALSE,FA_Widget_SetMax,FV_Widget_SetBoth,TAG_DONE),
            Child, LOD->parent = F_MakeObj(FV_MakeObj_Button,"/",FA_ChainToCycle,FALSE,FA_Widget_SetMax,FV_Widget_SetBoth,TAG_DONE),
    ...
    TAG_MORE,Msg))
    {
        ...
        F_Do(LOD->volumes,FM_Notify,FA_Widget_Pressed,FALSE,LOD->list,FM_Set,2,"FA_DOSList_Path","Volumes");
        F_Do(LOD->parent,FM_Notify,FA_Widget_Pressed,FALSE,LOD->list,FM_Set,2,"FA_DOSList_Path","Parent");
        F_Do(LOD->match,FM_Notify,FA_Widget_Selected,FV_Notify_Always,Obj,FM_Set,2,F_IDA(FA_FileChooser_FilterFiles),FV_Notify_Value);
        F_Do(LOD->icons,FM_Notify,FA_Widget_Selected,FV_Notify_Always,Obj,FM_Set,2,F_IDA(FA_FileChooser_FilterIcons),FV_Notify_Toggle);
...
//         F_Log(0,">> file (0x%08lx)(%s)",tag->ti_Data,tag->ti_Data);
        ...
        {
            F_Do(LOD->icons,FM_Set,FA_NoNotify,TRUE,FA_Widget_Selected,!item.ti_Data,TAG_DONE);
        ...
        case FA_FileChooser_FilterFiles:
        {
            if (item.ti_Data)
            {
                if (F_Get(LOD->pattern,FA_Widget_Hidden))
                {
                    F_Do(Obj,FM_MultiSet,FA_Widget_Hidden,FALSE,LOD->balance,LOD->pattern,NULL);
                }                
            }
            else
            {
                if (!F_Get(LOD->pattern,FA_Widget_Hidden))
                {
                    F_Do(Obj,FM_MultiSet,FA_Widget_Hidden,TRUE,LOD->balance,LOD->pattern,NULL);
                }
            }

            F_Do(LOD->match,FM_Set,FA_NoNotify,TRUE,FA_Widget_Selected,item.ti_Data,TAG_DONE);
