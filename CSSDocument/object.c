...
#else
///cssdocument_new_property
FCSSProperty * cssdocument_new_property(FClass *Class, FObject Obj, FCSSDeclaration *Declaration, STRPTR PropertyKey, uint32 PropertyKeyLength, STRPTR ValueKey, uint32 ValueKeyLength)
{
	struct LocalObjectData *LOD = F_LOD(Class,Obj);

	FCSSProperty *property = IFEELIN F_NewP(LOD->pool, sizeof (FCSSProperty) + ValueKeyLength + 1);

	if (property)
	{
		property->Next = Declaration->Properties;

		//property->Atom = (FAtom *) IFEELIN F_Do(Obj, F_RESOLVED_ID(OBTAINNAME), PropertyKey, PropertyKeyLength);
		property->Atom = (FAtom *) IFEELIN F_AtomObtainP(LOD->atoms_pool, PropertyKey, PropertyKeyLength);
		...
		property->Value = (STRPTR)((uint32)(property) + sizeof (FCSSProperty));

		IEXEC CopyMem(ValueKey, property->Value, ValueKeyLength);
		property->NumericValue = IFEELIN F_Do(Obj, F_RESOLVED_ID(NUMERIFY), property->Value, &property->NumericType);

		#ifdef DB_PARSE
		IFEELIN F_Log(0,"new property: (%s)(%s)",property->Name,property->Value);
		#endif

		Declaration->Properties = property;
	}
		...
		if (file)
		{
			...
			for (declaration = LOD->declarations ; declaration ; declaration = declaration->Next)
			{
				...
				if (property != NULL)
				{
					...
					{
						IDOS_ FPrintf(file, "\t%s: %s;\n", property->Atom->Key, property->Value);
					}
